...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, SparseArray<YtFile>> {
    ...
    private static final Pattern patCaption = Pattern.compile("https%3A%2F%2Fwww.youtube.com%2Fapi%2Ftimedtext(.*?)(%22|\\z)");
    ...
    private void parseVideoMeta(String getVideoInfo) throws UnsupportedEncodingException {
        ...
        String title = null, author = null, url = null, channelId = null;
        ...
        mat = patCaption.matcher(getVideoInfo);
        if (mat.find()) {
            url = URLDecoder.decode(mat.group(1), "UTF-8");
        }
